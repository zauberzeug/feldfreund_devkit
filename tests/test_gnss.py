...
async def test_rtk_lost(gnss_driving: System, gnss: GnssSimulation):
    ...
    await forward(until=lambda: gnss_driving.automator.is_paused)
    # robot should have stopped driving
    assert_point(gnss_driving.robot_locator.pose.point, rosys.geometry.Point(x=3.1, y=0))
    gnss._is_connected = True
    gnss._gps_quality = GpsQuality.RTK_FIXED
    # robot should continue driving
    await forward(until=lambda: gnss_driving.automator.is_stopped)
    assert_point(gnss_driving.robot_locator.pose.point, rosys.geometry.Point(x=10.0, y=0))
