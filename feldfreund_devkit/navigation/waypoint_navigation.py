...
class WaypointNavigation(rosys.persistence.Persistable):
    ...
    async def _run(self) -> None:
        ...
        if self.current_segment.use_implement:
            ...
            if not await self._follow_segment_until(implement_target):
                return
            # stop the robot, and let the implement do its thing.
            await self.driver.wheels.stop()
    ...
    async def _run_continuous(self) -> None:
        ...
        if not self.current_segment.use_implement:
            # non-working segment
            await self._drive_along_segment(linear_speed_limit=self.linear_speed_limit)
